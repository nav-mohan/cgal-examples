...
std::vector<Point> load_csv_points(const std::string& filename) {
...
void normalize_points(std::vector<Point>& pts) {
    if (pts.empty()) return;
...
std::vector<Point> build_delaunay_edges(const std::vector<Point>& pts, float max_length = -1.0f) {
    ...
    std::cout << "Delaunay vertices: " << T.number_of_vertices() << "\n";
    std::cout << "Delaunay edges: " << T.number_of_finite_edges() << "\n";
    ...
    std::cout << "Extracted " << lines.size()/2 << " edges.\n";
    return lines;
}
...
int main() {
    ...
    GLFWwindow* window = glfwCreateWindow(800, 600, "Bunny Point Cloud + Delaunay", NULL, NULL);
    ...
    std::vector<Point> points = load_csv_points("../bunny.csv");
    normalize_points(points);

    // perform triangulation
    std::vector<Point> delaunay_edges = build_delaunay_edges(points,0.2f);
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        ImGui::Checkbox("Show Delaunay Triangulation", &show_delaunay);

        if (ImGui::SliderFloat("Max edge length", &max_edge_length, 0.0f, 0.3f)) 
        {
            // toggle recompute_edges when slider changes
            recompute_edges = true;
        }

        if (recompute_edges) 
        {
            delaunay_edges = build_delaunay_edges(points, max_edge_length);
